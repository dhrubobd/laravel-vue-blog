...
<template>
    ...
    <div v-if="bookmarks.data.length > 0" class="space-y-4">
      <div v-for="bookmark in bookmarks.data" :key="bookmark.id" class="p-4 bg-white rounded-lg shadow-md hover:shadow-lg transition duration-200">
        <div class="flex items-start justify-between">
          <div class="flex items-center space-x-4">
            <img 
              v-if="bookmark.post.image" 
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">
                By <span class="font-medium">{{ bookmark.post.user.username }}</span> - {{ formatDate(bookmark.post.created_at) }}
              </p>
              <div class="flex items-center mt-2 space-x-2">
                <span v-for="tag in bookmark.post.tags" :key="tag.id" class="px-2 py-1 text-xs bg-blue-100 text-blue-600 rounded-full">
                  {{ tag.name }}
                </span>
              </div>
            </div>
      ...
      <div class="mt-6 flex justify-center space-x-2">
        <button 
          @click="changePage(bookmarks.prev_page_url)" v-show="bookmarks.prev_page_url"
          class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md cursor-pointer">
          Previous
        </button>
        
        <button 
          @click="changePage(bookmarks.next_page_url)" v-show="bookmarks.next_page_url"
          class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md cursor-pointer">
          Next
        </button>
      </div>
